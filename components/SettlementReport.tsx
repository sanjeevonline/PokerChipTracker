  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         <Card className="flex flex-col items-center justify-center text-center py-8 bg-gradient-to-br from-slate-800 to-slate-900">
            <div className="text-slate-400 uppercase text-xs tracking-widest mb-2">Total Buy-In</div>
            <div className="text-4xl font-bold text-white font-mono">{formatCurrency(report.totalBuyIn)}</div>
         </Card>
         <Card className="flex flex-col items-center justify-center text-center py-8 bg-gradient-to-br from-slate-800 to-slate-900">
            <div className="text-slate-400 uppercase text-xs tracking-widest mb-2">Total Chips</div>
            <div className="text-4xl font-bold text-emerald-400 font-mono">{formatCurrency(report.totalChips)}</div>
         </Card>
         <Card className={`flex flex-col items-center justify-center text-center py-8 ${report.discrepancy !== 0 ? 'bg-red-900/20 border-red-500/30' : 'bg-gradient-to-br from-slate-800 to-slate-900'}`}>
            <div className="text-slate-400 uppercase text-xs tracking-widest mb-2 flex items-center gap-2">
               {report.discrepancy !== 0 && <AlertTriangle size={14} className="text-red-400" />}
               Discrepancy
            </div>
            <div className={`text-4xl font-bold font-mono ${report.discrepancy === 0 ? 'text-slate-500' : 'text-red-500'}`}>
           ...
           <table className="w-full text-left border-collapse">
             ...
             <tbody className="divide-y divide-slate-700/50 text-sm">
               {report.players.map(p => (
                 <tr key={p.playerId} className="hover:bg-slate-700/20 transition-colors">
                   <td className="p-4 font-medium text-white">{p.name}</td>
                   <td className="p-4 text-right text-slate-300">{formatCurrency(p.totalBuyIn)}</td>
                   <td className="p-4 text-right">
                     {p.transfersIn > 0 && <span className="text-red-400 block text-xs">Borrowed: {formatCurrency(p.transfersIn)}</span>}
                     {p.transfersOut > 0 && <span className="text-emerald-400 block text-xs">Loaned: {formatCurrency(p.transfersOut)}</span>}
                     {p.transfersIn === 0 && p.transfersOut === 0 && <span className="text-slate-600">-</span>}
                   </td>
                   <td className="p-4 text-right font-medium text-slate-200">{formatCurrency(p.netInvested)}</td>
                   <td className="p-4 text-right font-mono text-slate-200">{formatCurrency(p.finalChips)}</td>
                   <td className={`p-4 text-right font-bold font-mono ${p.netProfit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
              ...
              return (
                <div key={tx.id} className="flex items-center justify-between p-3 bg-slate-900/40 rounded-lg border border-slate-700/30">
                   <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-full ${tx.type === TransactionType.BUY_IN ? 'bg-emerald-500/10 text-emerald-500' : 'bg-blue-500/10 text-blue-500'}`}>
